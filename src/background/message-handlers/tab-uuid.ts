...
export async function handleTabUuidReport(
  ...
  chrome.storage.local.get(uuid, async (result) => {
    if (result[uuid]) {
      console.log(
        `[Dynamic Tab Groups] UUID Tracker - Tab ${tabId} is restored (UUID: ${uuid}). Skipping move.`,
      );
    } else {
      ...
      try {
        ...
        if (!currentTab.active && currentTab.openerTabId === undefined) {
          console.log(
            `[Dynamic Tab Groups] UUID Tracker - Tab ${tabId} is a background tab (First-run restore). Skipping move.`,
          );
          return;
        }

        console.log(
          `[Dynamic Tab Groups] UUID Tracker - Brand new tab detected (UUID: ${uuid}). Moving to right.`,
        );
