...
export async function onAttached(tabId: number, attachInfo: { newWindowId: number; newPosition: number }) {
  ...
  if (uuid) {
    ...
    console.log(`[Dynamic Tab Groups] Tab ${tabId} attached to new window. Updated UUID ${uuid} to display: ${displayId}`);
  }
