...
export function onRemoved(tabId: number, removeInfo: { isWindowClosing: boolean; windowId: number }) {
  ...
  if (uuid) {
    ...
    console.log(`[Dynamic Tab Groups] Cleaned up UUID from storage for closed tab: ${uuid}`);
  }
